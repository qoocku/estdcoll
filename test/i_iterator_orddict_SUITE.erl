...
%%% @end
%%% ==========================================================================
-module (i_iterator_orddict_SUITE).
...
groups () ->
  ?BASE_MODULE:groups() ++
    [{orddict, [parallel], [{group, iterators}]}].

init_per_group (orddict, Config) ->
  Samples = orddict:from_list(lists:zip(lists:seq(1, 1000), lists:seq(1000, 1, -1))),
  [{iterator, (i_map_dict:new({orddict, Samples})):iterator()},
   {samples, Samples},
   {erl_mod, orddict},
   {fold_fun, fun
                (acc, _) -> 0;
